...
/**
 * ComputeNetWorthTrendUseCase — returns a real (date, totalNetWorth) series
 * over the last [monthsBack] months. Net worth at each month-end is:
 *
 *   sum(account.initialBalance)
 *     + Σ income — Σ expense — Σ transferFee (across all txns ≤ month-end)
 *     + currentGoldTotal + currentLandTotal + currentDepositTotal
 *     − currentDebtsTotal
 *
 * Gold / Land / Deposit / Debt values aren't historicized in V1 (we don't
 * snapshot them per month), so they contribute a constant offset at every
 * point. The visible trend therefore reflects account activity — the
 * dominant driver for most users — instead of synthetic dummy growth.
 */
class ComputeNetWorthTrendUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        val initialBalances = accounts.sumOf { it.initialBalance }
        val txns = txnRepo.observeRecent(10_000).first()
        ...
        return (monthsBack downTo 0).map { m ->
            ...
            val accountDelta = txns
                .filter { !it.date.isAfter(endOfMonth) }
                .sumOf {
                    when (it.type) {
                        TxnType.Income -> it.amount
                        TxnType.Expense -> -it.amount
                        TxnType.Transfer -> -(it.transferFee ?: 0L)
                        else -> 0L
                    }
                }
            endOfMonth to (initialBalances + accountDelta + staticOffset)
        }
    }
}
